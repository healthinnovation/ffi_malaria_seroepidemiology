...
plot_recent_exposure <- ffi_household_gps %>% 
  ...
  geom_sf(data = iquitos_centro,
          aes(shape = "Loreto Regional Hospital"),
          size = 3) +
  # ggimage::geom_image(data = iquitos_centro,
  #                     aes(
  #                       x = Longitude,
  #                       y = Latitude,
  #                       image = "01_data/processed/hospital.png"
  #                     )) +
  scale_color_gradientn(colors = colorPalette) +
  scale_shape_manual(values = c(
    "Households" = 19,
    "Loreto Regional Hospital" = 17
  )) + 
  labs(
  ...
  guides(
    color = guide_colorbar(order = 1),  # Set color legend to second position
    shape = guide_legend(order = 2)  # Set shape legend to first position
  ) +
  ...
  data %>% 
    ...
    mutate(
      global_g = map2(
        data_nosf,
        self,
        ~ globalG.test(.x$prevalence, .y)
      ),
      global_g = map_dfr(global_g,
                         broom::tidy)
    ) %>% 
    unpack(global_g) %>% 
    mutate(p.value = scales::pvalue(p.value)) %>% 
    select(ffi_is_district, 
           type_plasmodium,
           "GlobalG" = estimate1,
           p.value) 
}
...
```{r}
global_g_kmeans <- global_g(ffi_prevalence_by_household_sf, 1) %>% 
  left_join(
    global_g(ffi_prevalence_by_household_sf, 2),
    by = join_by(ffi_is_district, type_plasmodium),
    suffix = c(".k1", ".k2")
  ) %>% 
  left_join(
    global_g(ffi_prevalence_by_household_sf, 3),
    by = join_by(ffi_is_district, type_plasmodium)
  ) %>% 
  left_join(
    global_g(ffi_prevalence_by_household_sf, 4),
    by = join_by(ffi_is_district, type_plasmodium),
    suffix = c(".k3", ".k4")
   )  %>% 
  left_join(
    global_g(ffi_prevalence_by_household_sf, 5),
    by = join_by(ffi_is_district, type_plasmodium)
  ) %>% 
  rename("GlobalG.k5" = "GlobalG",
         "p.value.k5" = "p.value") %>% 
  mutate(
    k_best = apply(select(., starts_with("GlobalG")), 1, which.max)
  )
```
...
ffi_kmeans_preparation <- ffi_prevalence_kmeans %>% 
  mutate(
    data_nosf = map(
      data,
      ~ .x %>% 
...
ffi_w.getis <- ffi_kmeans_preparation %>% 
  mutate(
    LISA = map2(data_nosf,
               self,
    ...
    LISA4 = map2(data_nosf,
                 self4,
                 ~ localG(.x$prevalence, .y)),
    LISA_clust4 = map(LISA4,
                     ~ cut(., include.lowest = TRUE,
                           breaks = breaks, 
                           labels = labels)),
    LISA_label_orig4 = map(LISA4,
                          ~ attributes(.)$cluster),
    LISA4 = map(LISA4, as.numeric)
  ) %>% 
  select(
    ffi_is_district,
    #ffi_is_community,
    #ffi_is_health_facility_name,
    type_plasmodium:LISA_label_orig4
  ) %>% 
  unnest(cols = c(data_nosf, LISA, 
                  LISA_clust, LISA_label_orig,
                  LISA4, 
                  LISA_clust4, LISA_label_orig4))
```
...
```{r}
# Función de trazado
create_plot <- function(data, title, variable) {
  data %>% 
...
```{r}
combined_plot_total_k4 <- (plot1_k4 + plot2_k4) / (plot3_k4 + plot4_k4) +
  plot_layout(guides = "collect") 
```

```{r eval = FALSE}
ggsave("02_output/plots/Fig 5. Spatial Analysis for Type of Exposure raster with K4.png",
       combined_plot_total_k4,
       dpi = 400,
       width = 12,
       height = 14,
       device = grDevices::png)
